...
if ( is_woocommerce_active() ) {
			...
			foreach( $fields as $name => $values ) {
				...
				if ( in_array( $name, array(
					...
					if ( isset( $fields[ $name ] ) ) {
						...
						$fields[ $name ]['label']       = ! empty( $data[ $name ]['label'] ) ? $data[ $name ]['label'] : $fields[ $name ]['label'];
						$fields[ $name ]['placeholder'] = ! empty( $data[ $name ]['placeholder'] ) ? $data[ $name ]['placeholder'] : $fields[ $name ]['placeholder'];
	...
	function wc_checkout_fields_scripts() {
		...
		if ( is_checkout() ) {
			wp_enqueue_script( 'wc-checkout-editor-frontend', plugins_url( 'assets/js/checkout.js' , __FILE__ ), array( 'jquery', 'chosen', 'jquery-ui-datepicker' ), WC()->version, true );


			if ( ! wp_style_is( 'woocommerce_chosen_styles', 'enqueued' ) && version_compare( WOOCOMMERCE_VERSION, '2.3.0', '<' ) ) {
	...
	function wc_checkout_fields_validation( $posted ) {
		foreach ( WC()->checkout->checkout_fields as $fieldset_key => $fieldset ) {
			...
			if ( $fieldset_key == 'shipping' && ( WC()->cart->ship_to_billing_address_only() || $posted['shiptobilling'] || ( ! WC()->cart->needs_shipping() && get_option('woocommerce_require_shipping_address') == 'no' ) ) ) {
				continue;
			}
