...
function themex_one_fourth( $atts, $content = null ) {
   return '<ul><li><div class="threecol column">'.do_shortcode($content).'</div></li></ul>';
}

add_shortcode('one_fourth_last', 'themex_one_fourth_last');
function themex_one_fourth_last( $atts, $content = null ) {
   return '<ul><li><div class="threecol column last">'.do_shortcode($content).'</div></li></ul><div class="clear"></div>';
}
...
function themex_courses( $atts, $content = null ) {
	...
	$out='<div class="courses-listing clearfix course-viewpage">';
	$counter=0;
    $inc = 0;
	while($query->have_posts()){
		...
		$out.='<div class="'.$layout.'col column '. $class;
...
//Testimonials Slider
add_shortcode('testimonials_slider','themex_testimonials_slider');
function themex_testimonials_slider( $atts, $content = null ) {
    extract(shortcode_atts(array(
        'number' => '3',
        'order' => 'date',
        't_ids' => '',
    ...
    if(isset($t_ids) && !empty($t_ids)){
        ...
        $t_ids = explode(',', $t_ids);
        $query = new WP_Query( array( 'post_type' => 'testimonial', 'post__in' =>  $t_ids ) );
    ...
    $out='<div class="testimonials"> <ul class="bxslider">';
    while($query->have_posts()){
        $query->the_post();    
        ob_start();
        get_template_part('loop','testimonial');
...
//Blog Slider
add_shortcode('blogs_slider','themex_blogs_slider');
function themex_blogs_slider( $atts, $content = null ) {
    extract(shortcode_atts(array(
        'number' => '3',
        'order' => 'date',
    ), $atts));
    ...
    $out='<div class="blog"> <ul class="bxslider">';
    while($query->have_posts()){
        $query->the_post();    
        ob_start();
...
//Page
    add_shortcode('page_content','themex_page_content');
    function themex_page_content( $atts, $content = null ) {
        extract(shortcode_atts(array(
            'number' => '1',
            'order' => 'date',
            'id' => '0',
        ...
        $out='<div class="page-content">';
            ...
            //$out.= '<h6>'.$page_data->post_title.'</h6>';
                              
            $content = limit_content_string( $page_data->post_content, $id, $word );
            $out.= '<p>'.$content.'</p>';
...
function themex_tabs( $atts, $content = null ) {
	...
	if(is_array($titles)) {
		foreach($titles as $title) {
			$out .= '<li><div class="btn">'.trim($title).'</div></li>';
		}
	}
    $out .= '</ul>';
...
function themex_toggle( $atts, $content = null ) {	
	...
	$out='<div class="toggle-container faq-toggle"><div class="toggle-title"><div class="question">'.$title.'</div></div><div class="toggle-content"><p>'.do_shortcode($content).'</p></div></div>';	
	return $out;
}
...
function themex_block( $atts, $content = null ) {	
	...
	$out='<div class="widget"><div class="widget-title"><div class="formtitle">'.$title.'</div></div><div class="widget-content">'.do_shortcode($content).'</div></div>';	
    return $out;
}
